...
router.get('/my-bu', async (req, res, next) => {
  try {
    ...
    const result = await db.query(
      `SELECT * FROM assignment_statuses WHERE business_unit_id = ${placeholder} AND is_active = true ORDER BY display_order ASC`,
      [businessUnitId]
    );
    res.json({ data: result.rows || [] });
  } catch (error) {
    logger.error('Get my BU assignment statuses error:', error);
    next(error);
  }
});
...
router.get('/:business_unit_id', async (req, res, next) => {
  try {
    ...
    logger.error('Get assignment statuses error:', error);
    next(error);
  }
});
...
router.post('/:business_unit_id', requireAdmin, [
  ...
  try {
    ...
    const { status_code, status_name, description, color = '#17a2b8', badge_style = 'info', is_closed_status = false, display_order = 0 } = req.body;
    const dbType = process.env.DB_TYPE || 'postgresql';

    const checkPlaceholder = dbType === 'mysql' ? '?' : '$1';
    const existing = await db.query(
      `SELECT id FROM assignment_statuses WHERE business_unit_id = ${checkPlaceholder} AND status_code = ${dbType === 'mysql' ? '?' : '$2'}`,
      [business_unit_id, status_code]
    );
    if (existing.rows.length > 0) {
      return res.status(409).json({ error: { code: 'RESOURCE_CONFLICT', message: 'Assignment status with this code already exists' } });
    }

    let result;
    if (dbType === 'mysql') {
      await db.query(
        `INSERT INTO assignment_statuses (business_unit_id, status_code, status_name, description, color, badge_style, is_closed_status, display_order, created_by)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
        [business_unit_id, status_code, status_name, description || null, color, badge_style, is_closed_status, display_order, req.user.id]
      );
      result = await db.query(`SELECT * FROM assignment_statuses WHERE business_unit_id = ? AND status_code = ?`, [business_unit_id, status_code]);
    } else {
    ...
    logger.error('Create assignment status error:', error);
    next(error);
  }
});
...
router.patch('/:business_unit_id/:id', requireAdmin, async (req, res, next) => {
  try {
    ...
    if (is_closed_status !== undefined) { paramCount++; updates.push(`is_closed_status = ${dbType === 'mysql' ? '?' : `$${paramCount}`}`); params.push(is_closed_status); }
    ...
    if (dbType === 'mysql') {
      await db.query(`UPDATE assignment_statuses SET ${updates.join(', ')} WHERE id = ?`, params);
      const result = await db.query(`SELECT * FROM assignment_statuses WHERE id = ?`, [id]);
      res.json(result.rows[0]);
    } else {
      const result = await db.query(`UPDATE assignment_statuses SET ${updates.join(', ')} WHERE id = $${paramCount} RETURNING *`, params);
      res.json(result.rows[0]);
    }
  } catch (error) {
...
router.delete('/:business_unit_id/:id', requireAdmin, async (req, res, next) => {
  try {
    ...
    const dbType = process.env.DB_TYPE || 'postgresql';
    const placeholder = dbType === 'mysql' ? '?' : '$1';
    await db.query(`UPDATE assignment_statuses SET is_active = false WHERE id = ${placeholder}`, [id]);
    res.json({ message: 'Assignment status deleted successfully' });
  } catch (error) {
    logger.error('Delete assignment status error:', error);
    next(error);
  }
});
