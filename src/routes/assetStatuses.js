...
async function canManageBU(userId, businessUnitId) {
  try {
    ...
    const userResult = await db.query(
      `SELECT u.id, r.name as role_name, u.business_unit_id
       FROM users u
...
// GET /api/v1/asset-statuses/my-bu
  ...
  try {
    ...
    if (userResult.rows.length === 0 || !userResult.rows[0].business_unit_id) {
      return res.status(400).json({
        error: { code: 'NO_BUSINESS_UNIT', message: 'User is not assigned to a business unit' }
      });
    }
    ...
    const result = await db.query(
      `SELECT * FROM asset_statuses
       WHERE business_unit_id = ${placeholder} AND is_active = true
    ...
    logger.error('Get my BU asset statuses error:', error);
    next(error);
  }
});

// GET /api/v1/asset-statuses/:business_unit_id
router.get('/:business_unit_id', async (req, res, next) => {
  try {
    ...
    logger.error('Get asset statuses error:', error);
    next(error);
  }
});

// POST /api/v1/asset-statuses/:business_unit_id
  ...
  try {
    ...
    if (!hasPermission) {
      return res.status(403).json({
        error: { code: 'AUTHORIZATION_FAILED', message: 'You do not have permission to manage this business unit' }
      });
    }
    ...
    const existing = await db.query(
      `SELECT id FROM asset_statuses WHERE business_unit_id = ${checkPlaceholder} AND status_code = ${dbType === 'mysql' ? '?' : '$2'}`,
      [business_unit_id, status_code]
    );

    if (existing.rows.length > 0) {
      return res.status(409).json({
        error: { code: 'RESOURCE_CONFLICT', message: 'Asset status with this code already exists' }
      });
    }
    ...
    if (dbType === 'mysql') {
      await db.query(
        `INSERT INTO asset_statuses (business_unit_id, status_code, status_name, description, color, badge_style, display_order, created_by)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
        [business_unit_id, status_code, status_name, description || null, color, badge_style, display_order, req.user.id]
      );
      result = await db.query(`SELECT * FROM asset_statuses WHERE business_unit_id = ? AND status_code = ?`, [business_unit_id, status_code]);
    } else {
    ...
    logger.error('Create asset status error:', error);
    next(error);
  }
});

// PATCH /api/v1/asset-statuses/:business_unit_id/:id
router.patch('/:business_unit_id/:id', requireAdmin, async (req, res, next) => {
  try {
    ...
    if (dbType === 'mysql') {
      await db.query(`UPDATE asset_statuses SET ${updates.join(', ')} WHERE id = ?`, params);
      const result = await db.query(`SELECT * FROM asset_statuses WHERE id = ?`, [id]);
      res.json(result.rows[0]);
    } else {
      const result = await db.query(`UPDATE asset_statuses SET ${updates.join(', ')} WHERE id = $${paramCount} RETURNING *`, params);
      res.json(result.rows[0]);
    }
  } catch (error) {
    logger.error('Update asset status error:', error);
    next(error);
  }
});

// DELETE /api/v1/asset-statuses/:business_unit_id/:id
router.delete('/:business_unit_id/:id', requireAdmin, async (req, res, next) => {
  try {
    ...
    if (!hasPermission) {
      return res.status(403).json({
        error: { code: 'AUTHORIZATION_FAILED', message: 'You do not have permission' }
      });
    }

    const dbType = process.env.DB_TYPE || 'postgresql';
    const placeholder = dbType === 'mysql' ? '?' : '$1';
    await db.query(`UPDATE asset_statuses SET is_active = false WHERE id = ${placeholder}`, [id]);
    res.json({ message: 'Asset status deleted successfully' });
  } catch (error) {
    logger.error('Delete asset status error:', error);
    next(error);
  }
});
